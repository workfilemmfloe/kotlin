    ...
    fun eval(line: String): ReplEvalResult {
        ...
        try {
            ...
            fun SourceCode.Location.toCompilerMessageLocation() =
                CompilerMessageLocation.create(snippet.name, start.line, start.col, snippet.text)
            ...
            fun ResultWithDiagnostics<*>.reportToMessageCollector() {
                for (it in reports) {
                    ...
                    messageCollector.report(diagnosticSeverity, it.message, null)
                }
            }

            @Suppress("DEPRECATION_ERROR")
            val evalRes: ReplEvalResult = internalScriptingRunSuspend {
                val compileResult: ResultWithDiagnostics<LinkedSnippet<KJvmCompiledScript>> = compiler.compile(listOf(snippet), compilationConfiguration)
                when (compileResult) {
                    is ResultWithDiagnostics.Failure -> {
                        val incompleteReport = compileResult.reports.find { it.code == ScriptDiagnostic.incompleteCode }
                        val firstError =
                            compileResult.reports.find {
                        ...
                        when (evalResult) {
                            is ResultWithDiagnostics.Success -> {
                                when (val evalValue = evalResult.value.get().result) {
                                    ...
                                    is ResultValue.Error -> ReplEvalResult.Error.Runtime(evalValue.error.message ?: "", evalValue.error.let { it as? Exception })
                                    else -> ReplEvalResult.Error.Runtime("Error: snippet is not evaluated")
                                }
                            }
                            else -> {
                                val firstError = evalResult.reports.find { it.code == ScriptDiagnostic.unspecifiedError }
