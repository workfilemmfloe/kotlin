...
class InlineFunctionTransitiveHashCalculator {
    ...
    private val transitiveHashes = mutableMapOf<IrFunction, ICHash>()
    ...
    private fun getInlineFunctionTransitiveHash(f: IrFunction): ICHash = transitiveHashes.getOrPut(f) {
    ...
    fun updateTransitiveHashes(fragments: Collection<IrModuleFragment>): Map<IdSignature, ICHash> {
        ...
        return buildMap(transitiveHashes.size) {
            for ((function, hash) in transitiveHashes.entries) {
                function.symbol.signature?.let { put(it, hash) }
            }
        }
    }
}
